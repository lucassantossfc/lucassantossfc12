...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'FN:ZEUS\n' //NAMA LU
            + 'ORG:Owner Bot;\n' // NAMA CONTACT OWNER
            + 'TEL;type=CELL;type=VOICE;waid=556993899391:+55 19 99814-0665\n' // NOMER OWNER
            + 'END:VCARD'
prefix = '#'//PREFIX BOT
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}\nNão se esqueça de ler as regras e chamar um dos adms para escolher seu personagem\n\n\nSinta-se avontade no grupo:)*`
				...
				teks = `Thauzinhoo arrombado @${num.split('@')[0]}, Não volte por favor`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Aguarde, O Lucas e a fernanda esta fazendo⏳',
				success: '[ ✔ ]Achei teu corno(a)',
				...
				only: {
					...
					ownerG: '[❗] Esse Comando so pode ser usado por Lucas!',
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        // Options
                        const botName = 'Lucas&Fer'//Nama Bot
		...
		if (isGroupAdmins) return reply('_Não faça isso fernanda sua corna!_')
